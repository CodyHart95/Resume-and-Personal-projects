...
def roll(attack_dice, defend_dice):
    ...
    return attack_loss, defend_loss  #Return a tuple with number losses for each playe
...
def main(num_simulations):
    ...
    procSim = num_simulations // num_cores
    procSimMod = num_simulations % num_cores

    for i in range(num_cores):
        if procSimMod == 0:
            mp = multi.Process(target=run_simulations, args=[procSim,num_armies,num_attack_dice,num_defend_dice,que])
        else:
            mp = multi.Process(target=run_simulations, args=[procSim + 1, num_armies, num_attack_dice, num_defend_dice, que])
            procSimMod -= 1
        mp.start()
        mpList.append(mp)
    for p in mpList:
